...
/* ALL SELECTBOXES - selected value */
...
def convert_magnetic_moment_per_unit_mass(value, unit):
    if unit in ["A m² kg⁻¹", "A m² kg⁻¹", "A m^2/kg", "A m² kg-1"]:
        return value * 1, "emu g⁻¹"   
    elif unit in ["emu g⁻¹", "EMU"]:
        return value * 1, "A m² kg⁻¹" 
    else:
        return value, unit
...
def convert_mass_magnetic_susceptibility(value, unit):
    if unit in ["m³ kg⁻¹", "m³ kg⁻¹", "m^3/kg", "m³/kg"]:
        return value * (10**3 / (4 * math.pi)), "emu Oe⁻¹ g⁻¹"
    elif unit in ["emu Oe^-1 g^-1", "emu Oe^-1/g", "emu Oe⁻¹ g⁻¹"]:
        return value / (10**3 / (4 * math.pi)), "m³ kg⁻¹"
    else:
        return value, unit
